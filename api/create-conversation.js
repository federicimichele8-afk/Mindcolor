...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Cerca contenuti rilevanti in Pinecone
    const pc = new Pinecone({ apiKey: process.env.PINECONE_API_KEY });
    const index = pc.index('mindcolor-knowledge', process.env.PINECONE_HOST);
    const query = `colore ${colore_dominante} blocco ${ultimo_blocco} coaching`;
    const results = await index.namespace('youtube').searchRecords({ query: { inputs: { text: query }, topK: 3 } });
    
    const knowledge = results.result?.hits?.map(h => h.fields?.text || '').join('\n\n') || '';
...
CONTENUTI RILEVANTI DAL TUO METODO:
${knowledge}
